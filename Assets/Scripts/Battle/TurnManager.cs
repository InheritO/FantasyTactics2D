...
public class TurnManager : MonoBehaviour
{
    ...
    public void EndTurn()
    {
        if (turnOrder.Count == 0)
        {
            Debug.LogWarning("턴 순서가 비어있는 상태에서 EndTurn이 호출되었습니다. InitializeTurnOrder가 먼저 호출되었는지 확인하세요.");
            return;
        }

        if (turnOrder.Count == 1)
            return; // 세력이 하나만 남은 상태 (전투 종료 처리는 BattleOutcomeManager가 담당)
    ...
    // insertNext: true면 바로 다음 차례로 끼워넣음, false면 순서 맨 뒤에 추가
